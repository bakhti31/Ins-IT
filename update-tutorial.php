...
$id=0;
if ($_GET['id']=="") {
	header("Location: http://location/DTSB/tutorial.php");
}else{
  $id = $_GET['id'];
}
...
$konten=read('tutorial','*','WHERE id='.$_GET['id']);
$data=mysqli_fetch_assoc($konten);
$pengguna=read('user','id,username',"WHERE username='".$_SESSION['pengguna']."'");
...
if (isset($_POST['title'])) {
  ...
  if ($query) {
    ...
    header("Location: http://localhost/DTSB/update-tutorial.php?id=$id&error=1");
  }
}
